d3.slider = function module() {
  ...
  var div, min = 0, max = 100, svg1, svg1Group, value, classPrefix, axis, 
  height=40, rect,
  rectHeight = 12,
  tickSize = 6,
  ...
  function slider(selection) {
    selection.each(function() {
      ...
      var values = [value];
      dragger = svg1.selectAll(".dragger")
      .data(values)
      .enter()
      .append("g")
      .attr("class", "dragger")
  ...
  slider.move = function(pos) {
    ...
    // Move dragger
    svg1.selectAll(".dragger").data(values)
    .attr("transform", function(d) {
      return "translate(" + scale(d) + ")";
    });
    ...
    //   displayValue = d3.format(",.0f")(value);
    // }
    // svg1.selectAll(".dragger").select("text")
    // .text(displayValue);
   
    if (range) { 
      svg1.selectAll(".d3slider-rect-value")
      .attr("width", scale(value));
    }
